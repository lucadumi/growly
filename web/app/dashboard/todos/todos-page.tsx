  ...
  const handleDrop = async (event: React.DragEvent, targetStatus: Status) => {
    ...
    try {
      ...
      if (reopenedFromCompleted) {
        ...
        // newly completed
        addXP(XP_PER_TODO, "todo", { label: current?.title });
      }
    } catch (error) {
      console.error(error);
      setTodos((prev) =>
  ...
  const handleDeleteTodo = async (id: string) => {
    ...
    try {
      ...
      setCollections((prev) =>
        prev.map((collection) => ({
          ...collection,
          todoIds: collection.todoIds.filter((tid) => tid !== id),
        })),
      );
      closeTodoSlideOver();
    } catch (error) {
        ...
        <div className="relative z-10">
          ...
          <div className="w-full h-auto lg:px-6 xl:px-8 2xl:px-28">
            <div className="space-y-4">
                            ...
                            items.map((todo) => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`group relative block w-full text-left rounded-xl border border-gray-100 hover:border-gray-300 bg-white lg:p-2 xl:p-3 transition ${
              ...
              {!isCollectionView ? (
                <div className="lg:space-y-5 xl:space-y-6 lg:mt-5 xl:mt-6 relative overflow-hidden">
