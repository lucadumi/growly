...
export const XPProvider: React.FC<XPProviderProps> = ({ children }) => {
  ...
  const addXP = useCallback(
    (
      ...
      // When undoing (negative XP), remove the matching positive entry instead
      // of adding a new negative one.
      if (amount < 0 && metadata.label) {
        const hasMatch = activityLogRef.current.some(
          (e) => e.source === source && e.xp > 0 && e.label === metadata.label
        );
        if (hasMatch) {
          setActivityLog((prev) => {
          ...
          setTotalXP((prev) => Math.max(0, (prev ?? 0) + amount));
          setTodayXP((prev) => Math.max(0, (prev == null ? 0 : prev) + amount));
          setStreakBonus((prev) =>
            Math.max(
              0,
              Math.min(MAX_STREAK_BONUS, (prev == null ? 0 : prev) + Math.floor(amount / 2))
            )
          );
          return;
        }
